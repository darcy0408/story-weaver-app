...
def _build_age_instruction_block(age: int) -> str:
    ...
    return (
        f"AGE {age}: {guidelines['length_guideline']}, "
        f"{guidelines['vocabulary_level']}, "
        f"{guidelines['sentence_structure']}. "
        f"EXAMPLES: {guidelines['vocabulary_examples']}. "
        f"{guidelines['special_instructions']}"
    )
...
def _truncate_prompt_if_needed(prompt: str, max_tokens: int = 1000000) -> str:
    """Truncate prompt if it exceeds token limit to prevent API errors"""
    estimated_tokens = _estimate_token_count(prompt)
    if estimated_tokens <= max_tokens:
        return prompt

    app.logger.warning(f"Prompt too long ({estimated_tokens} tokens), truncating to fit {max_tokens} token limit")

    # Keep the essential parts and truncate less critical sections
    lines = prompt.split('\n')
    essential_parts = []
    optional_parts = []

    for line in lines:
        # Essential parts that must be kept
        if any(keyword in line.upper() for keyword in ['STORY DETAILS:', 'CHARACTER:', 'AGE ', 'THERAPEUTIC', 'FEELINGS']):
            essential_parts.append(line)
        else:
            optional_parts.append(line)

    # Combine essential parts first, then add optional parts until we hit the limit
    result = '\n'.join(essential_parts)
    remaining_tokens = max_tokens - _estimate_token_count(result)

    for part in optional_parts:
        part_tokens = _estimate_token_count(part)
        if remaining_tokens > part_tokens:
            result += '\n' + part
